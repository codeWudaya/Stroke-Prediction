...
function App() {
  ...
  const [strokePrediction, setStrokePrediction] = useState(null); // State for storing prediction result
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Prediction Result:", result.stroke); // Log the prediction result
    setStrokePrediction(result.stroke); // Update state with the result from the backend
  };

  // Function to determine text color
  const getTextColor = () => {
    if (strokePrediction === 1) return "red"; // High chance of stroke
    if (strokePrediction === 0) return "green"; // Low chance of stroke
    return "black"; // Default color if not determined
  };
  ...
  return (
    <div className="app-container">
      ...
      <div className="form-container">
        ...
        {/* Conditionally render the result with color based on prediction */}
        {strokePrediction !== null && (
          <div className="result" style={{ color: getTextColor() }}>
            <h2>Prediction Result:</h2>
            {strokePrediction === 1 ? (
              <p>Yes, high chance of stroke</p>
            ) : (
              <p>No, less chance of stroke</p>
            )}
          </div>
        )}
      </div>
